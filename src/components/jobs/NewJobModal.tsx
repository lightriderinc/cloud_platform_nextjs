...
export default function NewJobModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="default-radius w-full max-w-md bg-white p-8 shadow-xl"
      ...
      >
        <div className="mb-6 flex items-center justify-between">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-900 text-white hover:bg-gray-700"
          >
            ×
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {isError && (
            <p className="text-sm text-red-500">Failed to submit job. Please try again.</p>
          )}
