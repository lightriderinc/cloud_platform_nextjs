...
// omitted from the summary; detailed metrics live in the card's modal.
export default function BackendCard({ backend }: { backend: Backend }) {
  const { name, type, status, qubits, provider } = backend;

  return (
    <TiltCard className="flex h-full cursor-pointer flex-col gap-3 rounded-lg border border-gray-200 bg-white p-4">
